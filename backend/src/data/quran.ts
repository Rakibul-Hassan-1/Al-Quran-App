import type { Ayah, QuranData, Surah } from "../types";

// Surah metadata (name, englishName, revelation place, ayah count)
const SURAH_META: Array<{
  number: number;
  name: string;
  englishName: string;
...
export async function loadQuranData(): Promise<QuranData> {
  ...
  try {
    // Fetch Arabic and English editions first (required)
    ...
    // Attempt to auto-detect a Bangla edition by querying the editions list.
    // The editions endpoint may be available at /v1/editions or /v1/edition depending on the API.
    async function findBanglaSlug(): Promise<string | null> {
      const listPaths = [
        "https://api.alquran.cloud/v1/editions",
      ...
      for (const path of listPaths) {
        try {
          ...
          const editions = data.data || data; // some endpoints wrap in data
          if (!Array.isArray(editions)) continue;

          // Look for entries with language 'bn' or name containing 'Bengali'/'Bangla'
          const candidate = editions.find((e: any) => {
            const lang = (e.language || "").toLowerCase();
            const name = (e.englishName || e.name || "").toLowerCase();
            return (
              lang.startsWith("bn") ||
          ...
          if (candidate && candidate.identifier) return candidate.identifier;
          if (candidate && candidate.slug) return candidate.slug;
          if (candidate && candidate.englishName)
            return candidate.englishName.toLowerCase();
        } catch (e) {
          // ignore and try next
        }
      }

      // fallback to a small set of known slugs
      const fallback = ["bn.bengali", "bn.hossain", "bn.ahmedali"];
      for (const slug of fallback) {
        try {
          const res = await fetch(`https://api.alquran.cloud/v1/quran/${slug}`);
    ...
    const banglaSlug = await findBanglaSlug();
    let banglaData: any = null;
    if (banglaSlug) {
      try {
        const res = await fetch(
          `https://api.alquran.cloud/v1/quran/${banglaSlug}`,
        );
        if (res.ok) {
          banglaData = await res.json();
          console.info(`Using Bangla edition: ${banglaSlug}`);
        } else {
          console.warn(`Bangla edition ${banglaSlug} returned ${res.status}`);
        }
      } catch (e) {
        console.warn(`Failed to fetch Bangla edition ${banglaSlug}:`, e);
      }
    } else {
      console.info(
        "No Bangla edition detected from API; continuing without Bangla.",
      );
    }

    const arabicSurahs = arabicData.data.surahs;
    const englishSurahs = englishData.data.surahs;
    const banglaSurahs = banglaData ? banglaData.data.surahs : null;

    const surahs: Surah[] = arabicSurahs.map(
      (arabicSurah: any, index: number) => {
        const englishSurah = englishSurahs[index];
        const meta = SURAH_META[index];

        const ayahs: Ayah[] = arabicSurah.ayahs.map(
          (arabicAyah: any, ayahIndex: number) => ({
            number: arabicAyah.numberInSurah,
            numberInQuran: arabicAyah.number,
            text: arabicAyah.text,
            translation: englishSurah.ayahs[ayahIndex].text,
            banglaTranslation: banglaSurahs
              ? banglaSurahs[index].ayahs[ayahIndex].text
              : undefined,
    ...
    // Return minimal fallback with metadata only
    return {
      surahs: SURAH_META.map((meta) => ({
        ...meta,
        ayahs: [],
      })),
    };
